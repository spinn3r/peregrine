 ...
 * 
 *
 * Mon Nov 28 2011 01:04 AM (burton@spinn3r.com): with or changes to work
 * with a ChannelBuffer which has a 5% performance hit.
 *
 */
public class PureJavaCrc32C implements Checksum {
  ...
  public void update( ChannelBuffer buff, int off, int len) {
    int localCrc = crc;
    while(len > 7) {
        int c0 = buff.getByte(off++) ^ localCrc;
        int c1 = buff.getByte(off++) ^ (localCrc >>>= 8);
        int c2 = buff.getByte(off++) ^ (localCrc >>>= 8);
        int c3 = buff.getByte(off++) ^ (localCrc >>>= 8);
        localCrc = (T8_7[c0 & 0xff] ^ T8_6[c1 & 0xff])
            ^ (T8_5[c2 & 0xff] ^ T8_4[c3 & 0xff]);
        
        localCrc ^= (T8_3[buff.getByte(off++) & 0xff] ^ T8_2[buff.getByte(off++) & 0xff])
            ^ (T8_1[buff.getByte(off++) & 0xff] ^ T8_0[buff.getByte(off++) & 0xff]);
        
        len -= 8;
    }
    while(len > 0) {
        localCrc = (localCrc >>> 8) ^ T8_0[(localCrc ^ buff.getByte(off++)) & 0xff];
        len--;
    }
